import { Store } from '../../types';
    ...
    const existingBeverageStoreNames = new Set(beverageStores.map(store => store.name.toLowerCase()));

    const newBeverageStores: Store[] = [
      {
        id: 'beverage-pc-cola',
    ...
    export const beverageStores = [
      ...newBeverageStores.filter(store => !existingBeverageStoreNames.has(store.name.toLowerCase()))
    ];
